...
# AC: it sums across all cells of a given cell type, but it does not collapse segments within a section., output[channel][cell_type][section][segment] = summed_time_series
...
# AC: This one collapses all segments within each section. output[channel][cell_type][section] = summed_time_series
...
#AC: The two previous functions aggregate one chosen channel across all cells of each requested cell type, separately for L2_pyramidal and L5_pyramidal.
# They return:
#{
#    "ina_hh2": {
#        "L2_pyramidal": ...,
...
#    }
#}
# In other words: These functions aggregate a specific transmembrane current channel
# over all cells of the specified cell types, returning separate outputs for L2_pyramidal
#  and L5_pyramidal; one function preserves segment-level resolution, while the other collapses currents to the section level.
# crucial for what we want to do for LFP/CSD

#AC: This function is a wrapper that runs the section-level aggregation for multiple channels, then collects everything into one dictionary.
#channel_section_data["ina_hh2"] = {
#    "L2_pyramidal": ...,
#    "L5_pyramidal": ...,
#}
#for each channel -> for each cell type -> for each section -> summed time series
...
#AC: This whole block is a custom visualization pipeline that takes section endpoint geometry plus section diameters and turns them into a flattened 2D schematic of an L5 pyramidal neuron, either from manually specified endpoints or from the actual HNN cell object.
