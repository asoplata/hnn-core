...
def _cell_L5Pyr(override_params, pos=(0.0, 0.0, 0), gid=0.0):
    ...
    sections = _get_dends(
        p_all, cell_type="L5Pyr", section_names=section_names, v_init=v_init
    )
    ...
    synapses = _get_syn_props(
        p_all, "L5Pyr", syn_types=["ampa", "nmda", "gabaa", "gabab"]
    )
    return Cell(
        "L5Pyr",
        pos,
...
def _get_interneuron_soma(cell_name, v_init=-69):
    ...
    return Section(L=20.0, diam=20.0, cm=1, Ra=200.0, end_pts=end_pts, v=v_init)
